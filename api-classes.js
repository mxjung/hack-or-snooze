...
class StoryList {
  ...
  async addStory(user, newStory) {
    ...
    // query the /stories endpoint (no auth required)
    const existingStories = await axios.get(`${BASE_URL}/stories`);

    // turn the plain old story objects from the API into instances of the Story class
    const stories = existingStories.data.stories.map(story => new Story(story));

    // // Initialize new story as story class
    // console.log(response.data.story);
    // const newStoryInstance = new Story(response.data.story);

    // // Append out new story to stories
    // stories.push(newStoryInstance);
